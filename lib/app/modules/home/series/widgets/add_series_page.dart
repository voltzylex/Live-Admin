...
class _AddSeriesPageState extends State<AddSeriesPage> {
  ...
  Widget seriesDetails(SeriesController ser) {
    return Row(
      ...
      children: [
        // Image Upload
        seriesCoverImage(ser),
        const SizedBox(width: 40),
        // Text Fields for Series Name and Description
        seriesTextFields(ser),
      ],
    );
  }

  // Widget for series image upload section
  Widget seriesCoverImage(SeriesController ser) {
  ...
  // Widget for series text fields (name and description)
  Widget seriesTextFields(SeriesController ser) {
    return Expanded(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
  ...
  // Widget for each Season Card
  Widget seasonCard(SeriesController ser, int seasonIndex) {
    return Card(
      color: AppColors.white.withOpacity(0.1),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Expanded(
                    child: Text("Season ${seasonIndex + 1}",
            ...
            if (ser.seasons[seasonIndex].episodes.isNotEmpty)
              for (int episodeIndex = 0;
                  episodeIndex < ser.seasons[seasonIndex].episodes.length;
                  episodeIndex++)
                episodeCard(ser, seasonIndex, episodeIndex),
            TextButton.icon(
  ...
  // Widget for each Episode Card
  Widget episodeCard(SeriesController ser, int seasonIndex, int episodeIndex) {
    return Card(
      color: AppColors.white.withOpacity(0.2),
      margin: const EdgeInsets.symmetric(vertical: 5),
      child: ListTile(
        leading: GestureDetector(
          onTap: () => ser.pickEpisodeImage(context, seasonIndex, episodeIndex),
          child: Container(
            width: 60,
            height: 60,
        ...
        title: TextField(
          controller: ser.getEpisodeTitleController(seasonIndex, episodeIndex),
          decoration: const InputDecoration(hintText: "Episode Title"),
        ).paddingAll(5),
        subtitle: TextField(
          controller:
              ser.getEpisodeDescriptionController(seasonIndex, episodeIndex),
          decoration: const InputDecoration(hintText: "Episode Description"),
          maxLines: 3,
        ).paddingAll(5),
        trailing: IconButton(
          icon: const Icon(Icons.delete, color: Colors.red),
  ...
  // Widget for the Action Buttons (Save/Cancel)
  Widget actionButtons(SeriesController ser) {
    return Row(
